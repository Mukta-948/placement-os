...
import type { MasterySignalSink } from "./learning-progress.service.js";

export class InterviewOrchestrator {
  constructor(private readonly sessions: InterviewSessionService, private readonly questions: QuestionStrategy, private readonly evaluator: AnswerEvaluator, private readonly difficulty: AdaptiveDifficultyPolicy, private readonly progress?: MasterySignalSink, private readonly now: () => Date = () => new Date()) {}
  ...
  private async answer(session: InterviewSession, answer: string, profile: UserProfile, context: ConversationContext): Promise<string> {
    ...
    if (completedTurns.length >= session.questionLimit) { const completed = { ...session, status: "completed" as const, turns: completedTurns, currentQuestion: undefined, metrics: undefined, completedAt: now, lastInteractionAt: now }; await this.sessions.save(completed); await this.progress?.recordMasterySignal(profile.userId, { topic: session.currentQuestion!.topic, score: evaluation.score, confidence: evaluation.confidence, occurredAt: now, source: "interview" }).catch(() => undefined); return this.feedback(evaluation) + "\n\nInterview complete. Great work."; }
    ...
    await this.progress?.recordMasterySignal(profile.userId, { topic: session.currentQuestion!.topic, score: evaluation.score, confidence: evaluation.confidence, occurredAt: now, source: "interview" }).catch(() => undefined);
